...
# ╔═╡ a5970b2e-0e88-4668-964f-4be3d0f01e66
using Enzyme
...
$$f\colon \mathbb{R} \to \mathbb{R}, \qquad f(x) = x^2 - y$$

mit Parameter $y > 0$. Die positive Nullstelle von $f$ ist $\sqrt{y}$,
die negative Nullstelle ist $-\sqrt{y}$.
"""

# ╔═╡ 62eb15cf-1d4e-4f5f-b28a-0da9e95404b2
md"""
`T` = $(@bind T Select([Float32, Float64, BigFloat], default = Float64))
"""

# ╔═╡ 4b7df0c1-3b96-4b45-b793-13da1c087968
md"""
``y`` = $(@bind y Slider(range(parse(T, "0.01"), 10, step = parse(T, "0.01")), default = 2, show_value = true))
"""
...
let
	...
	xmin = ifelse(y < 1, y, one(y))
	xmax = ifelse(y > 1, y, one(y))
	x = range(xmin, xmax, length = 10^3)
	lines!(ax, x, @. x^2 - y)
	scatter!(sqrt(y), zero(y); marker = :cross)
	fig
end

# ╔═╡ 20dd20c0-853a-4844-bf87-ac476c95dd92
md"mit ``\sqrt{y} \approx`` $(sqrt(y))"

# ╔═╡ 9907266d-12e0-47ab-a2ee-9da6c7e1b116
md"""
### Fazit

Das Heron-Verfahren (Newton-Verfahren) ist klar besser als das Bisektions-Verfahren
in diesem Fall. Statt den Fehler in jedem Schritt zu halbieren verdoppelt das
Newton-Verfahren die Anzahl der korrekten Stellen in jedem Schritt ungefähr.
"""

# ╔═╡ 45087012-86c3-4b5b-99fd-a29fdde79f5b
md"""
## Beispiel: Globale Konvergenz

Wir betrachten

$$f(x) = \mathrm{e}^x - 1$$

auf der reellen Achse. Für eine solche konvexe und monoton steigende
Funktion kann man zeigen, dass das Newton-Verfahren global konvergiert.
"""

# ╔═╡ 899eb552-d912-4acc-9517-a491cd773492
f1(x) = exp(x) - 1

# ╔═╡ 2c1fc67b-bacb-4f52-a02a-e0e371f74293
md"""
...
# ╔═╡ a14deaae-38ae-44a6-94de-edfe3c41c122
f2(x) = x^3 - 2x + 2

# ╔═╡ 19b0e116-a333-4812-912f-8fb78f61aafd
md"""
`maxiter_f2` = $(@bind maxiter_f2 Slider(0:30, default = 0, show_value = true))
"""

# ╔═╡ 8985a349-a46b-4754-99da-0b2a5db58372
md"""
## Beispiel: Minimierung einer Funktion

Zum Finden von Extremstellen wie beim Minimierungsproblem
...
# ╔═╡ f1f8726c-901c-4970-b3e0-b507ef793b2c
f3(x) = exp(sinpi(x)) + x^2

# ╔═╡ ff0b5472-a479-4a33-82de-1643737af356
f3′(x) = first(autodiff(Forward, f3, Duplicated(x, one(x))))

# ╔═╡ 25550c08-fa01-42ef-80ca-744e4cd0f270
md"""
...
let y = y
	...
	heron = accumulate(steps; init = y) do a, _
		heron_step(a, y)
	end
	...
	data = hcat(heron, abs.(heron .- sqrt(y)), 
				bisection_a, bisection_b, bisection_b - bisection_a)
	header = ["Heron", "Heron Fehler", "Bisektion aₙ", "Bisektion bₙ", "bₙ - aₙ"]
...
# ╔═╡ 4d26e010-ea6f-40e3-a7a1-c2b20f883f21
function newton(f, x0, maxiters)
	Jf = x -> first(autodiff(Forward, f, Duplicated(x, one(x))))
	x = copy(x0)
...
# ╔═╡ a9a4dfea-cf51-4638-ac36-45c4638a431e
newton(f1, -2.0, maxiter_f1)

# ╔═╡ 8513bbe9-30f5-4bf5-99c6-84a8a769cabc
x2_1 = newton(f2, -0.7, maxiter_f2)

# ╔═╡ b7769d0a-67b4-492d-9514-8aea773811e0
x2_2 = newton(f2, 0.0, maxiter_f2)

# ╔═╡ 8048ff73-b545-40fd-9ddf-9d82a832be0e
let
	fig = Figure()
	ax = Axis(fig[1, 1]; xlabel = L"x", ylabel = L"f(x)")
	
	x = range(-2.0, 2.0; length = 100)
	lines!(ax, x, f2.(x); label = L"f(x)")
	
	x0 = range(-1.0, 1.5; length = 100)
	lines!(ax, x0, 2 .- 2 .* x0; label = L"Tangente bei $x = 0$")
	
	x1 = range(-0.5, 1.5; length = 100)
	lines!(ax, x1, 1 .+ (x1 .- 1); label = L"Tangente bei $x = 1$")
	
	let x = x2_1
		y = f2(x)
		scatter!(ax, x, y)
		text!(ax, "Startwert 1"; position=(x, y), space = :data)
	end
	
	let x = x2_2
		y = f2(x)
		scatter!(ax, x, y)
		text!(ax, "Startwert 2"; position=(x, y), space = :data)
	end

	xlims!(ax, low = -2.1, high = 2.1)
	ylims!(ax, low = -2.1, high = 7.0)
	axislegend(position = :lt)
	scatter!([0, 1], [2, 1]; color = :gray, label = "", marker = :+)

	fig
end

# ╔═╡ 9cb6ed9a-6e82-44fc-812d-69a657b37c4f
x3_1 = newton(f3′, -0.7, maxiter_f3)

# ╔═╡ 0c101844-26dc-4e33-a27f-5f9e7906e3c8
x3_2 = newton(f3′, 1.5, maxiter_f3)

# ╔═╡ 9a93a088-71bc-4c8f-bec6-49bf22fd3202
let
	fig = Figure()
	ax = Axis(fig[1, 1]; xlabel = L"x", ylabel = L"f(x)")
	
	x = range(-2.0, 2.0; length = 100)
	lines!(ax, x, f3.(x); label = L"f(x)")
...
manifest_format = "2.0"
project_hash = "a43e7b07c4545d7c60ea48aefbe3beffc72805cf"

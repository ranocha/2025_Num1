...
# ╔═╡ 42501a1c-0ca8-4b77-96f8-194b52c3a81a
using Symbolics

# ╔═╡ e6c64c80-773b-11ef-2379-bf6609137e69
md"""
# 4.7 Nullstellen von Polynomen

Als erstes betrachten wir das Beispiel

$$p(x) = \prod_{i=1}^{20} (x- i)$$

von Wilkinson, das wir wie folgt ausmultiplizieren können.
"""

# ╔═╡ fd74447d-399f-4a03-b256-ca0b977ac732
let
	@variables x
	p = prod(x - i for i in 1:20) |> expand
end

# ╔═╡ 57940f0d-ba0c-42dd-a571-e4b9fc465bbd
md"""
Als nächstes formulieren wir das Polynom als

$$p(x) = \sum_{k=0}^{20} c_k x^k$$

und modifizieren den Koeffizienten $c_{19}$ von $x^{19}$ leicht.
"""

# ╔═╡ e8970a63-256a-44d7-8a23-464cd886b3bd
md"""
``c_{19}`` = $(@bind c19 Slider(range(-210 - 2.0^(-30), -210 + 2.0^(-30), length = 51), default = -210, show_value = true))
"""

# ╔═╡ 70835c23-3d70-48f3-a0a5-64f59e120fbd
let
	@variables x
	p = prod(x - BigFloat(i) for i in 1:20) |> expand
	d = first(polynomial_coeffs(p, (x,)))
	c = [float(d[x^i]) for i in 0:20]
	c[19 + 1] = c19

	fig = Figure()
	ax = Axis(fig[1, 1]; xlabel = L"x", ylabel = L"p(x)")
	x = range(0.0, 21.0, step = 1.0e-3)
	lines!(ax, x, evalpoly.(x, (c,)))
	ylims!(ax, -1.0e13, 1.0e13)

	fig
end

# ╔═╡ a93cbbb4-d471-419f-a6d7-c7835a72f1fd
md"""
Das ist jedoch im Wesentlichen ein Problem der Basis! Wenn wir stattdessen
beispielsweise die Werte des Polynoms an Chebyshev-Extremstellen als Basis
verwenden und diese mit $\Delta \xi_i$ stören, wobei $\xi_i$ normalverteilt
sind, dann erhalten wir das folgende Bild.
"""

# ╔═╡ da99c2b3-808c-4f54-b045-f15c96c66cde
md"""
``Δ`` = $(@bind Δ Slider(vcat(0, 10.0 .^ (-16:20)), default = 0, show_value = true))
"""
...
# ╔═╡ 1de3dfc9-ffea-44a6-9d27-2b9a741613d7
...
# ╔═╡ cf2fad88-9cf5-49e8-84c0-b16428a6a2ee
...
# ╔═╡ afed3762-25cd-4768-b4b5-47a87946343a
"""
...
# ╔═╡ 84061837-b3bc-4dd0-9e7d-c1b10899a0bc
"""
...
# ╔═╡ c09ac635-0fc5-4f73-bd3b-a33e06dd2f48
function interpolate(x, nodes, values)
    weights = barycentric_weights(nodes)
    return interpolate(x, nodes, values, weights)
end

# ╔═╡ b7f7d936-dce7-4bbe-a8f1-e3570ef3e63e
let
	nodes = 0:20
	values = zeros(length(nodes))
	values[begin] = factorial(20)
	values += Δ * randn(length(values))
	# values[begin] += Δ
	...
	fint = interpolate(x, float.(nodes), values)
	lines!(ax, x, fint)
	ylims!(ax, -1.0e13, 1.0e13)
...
manifest_format = "2.0"
project_hash = "79db2fba71232da3c4dd83e17efe516546f0bec1"

...
using TestImages, MosaicViews, ImageShow, ImageEdgeDetection, Images
...
md"""
# 5.11 Bildverarbeitung mit Analysis und partiellen Differentialgleichungen
...
[Jakob Emanuel Handmann](https://commons.wikimedia.org/wiki/File:Leonhard_Euler.jpg), Public domain, via Wikimedia Commons


### Joseph-Louis Lagrange

![Joseph-Louis Lagrange](https://upload.wikimedia.org/wikipedia/commons/4/46/%D0%9B%D0%B0%D0%B3%D1%80%D0%B0%D0%BD%D0%B6.jpg)


[http://www.hrono.ru/biograf/bio_l/lagranzhzh.html](https://commons.wikimedia.org/wiki/File:%D0%9B%D0%B0%D0%B3%D1%80%D0%B0%D0%BD%D0%B6.jpg), Public domain, via Wikimedia Commons

"""

# ╔═╡ 750a6e5f-39ad-4f25-8ef0-179df8563190
md"""
## Verschmelzen beider Gemälde

Als nächstes wollen wir die beiden Gemälde möglichst übergangslos verschmelzen.
Konkret wollen wir das Gemälde von Euler als Ausgangspunkt
nehmen aber das Gesicht mit dem von Lagrange ersetzen. Dazu laden wir als erstes
beide Bilder herunter.
"""

# ╔═╡ b2974e5d-46b4-4082-95ca-64d1a2033f2f
img_lagrange_original = let
	load(download("https://upload.wikimedia.org/wikipedia/commons/4/46/%D0%9B%D0%B0%D0%B3%D1%80%D0%B0%D0%BD%D0%B6.jpg"))
end;

# ╔═╡ 5c4be033-9449-4a96-99e3-f65c5219b12c
img_euler_original = let
	load(download("https://upload.wikimedia.org/wikipedia/commons/d/d7/Leonhard_Euler.jpg"))
end;

# ╔═╡ 55592c79-76b4-4caa-baa7-fba547d338ca
md"""
Als nächstes skalieren wir die Bilder so, dass beide die gleiche Größe haben.
"""

# ╔═╡ 8a4c86af-f4db-4369-8ad1-14ac5fa75192
img_lagrange_original_cropped = map(RGB{Float64}, img_lagrange_original)

# ╔═╡ f4e62b63-3c0b-4789-a420-7914c7e95ea0
img_euler_original_cropped = map(RGB{Float64}, imresize(
	img_euler_original[101:end, 51:(end-50)],
	size(img_lagrange_original_cropped)))

# ╔═╡ 96c706e1-6208-47c9-86e0-225fd3729382
md"""
Jetzt können wir das Gesicht von Lagrange auf das Gemälde von Euler kopieren. Das sieht natürlich sehr stümperhaft aus.
"""
...
md"""
Durch eine einfache Bildverarbeitung, die auf Analysis basiert, können wir die Bilder jedoch relativ gut aneinanderfügen - entweder als Euler-Lagrange oder
als Lagrange-Euler.
"""
...
numerische Diskretisierungen davon sowie das Lösen großer und dünnbesetzter
...
function osmosis_matrix(image, drift_vector_field)
	n, m = size(image)
	d1, d2 = drift_vector_field

	rows = Vector{Int}()
	cols = Vector{Int}()
	vals = Vector{real(eltype(image))}()
	...
	for j in 1:m, i in 1:n
		...
		# u_{i, j} influences u_{ij}
		push!(rows, idx_i0j0)
		push!(cols, idx_i0j0)
		# -4 u_{ij} / h²
		v = -4 * one(eltype(vals))
		if i < n
			# -d{1, i+1/2, j} u_{ij} / 2h
			v = v - d1[i, j] / 2
		else
			# i == n, contribution from u_{i+1, j}
			v = v + 1
		end
		if i > 1
			# +d{1, i-1/2, j} u_{ij} / 2h
			...
			# i == 1, contribution from u_{i-1, j}
			v = v + 1
		end
		if j < m
			# -d{2, i, j+1/2} u_{ij} / 2h
			v = v - d2[i, j] / 2
		else
			# j == m, contribution from u_{i, j+1}
			v = v + 1
		end
		if j > 1
			# +d{2, i, j-1/2} u_{ij} / 2h
			...
			# j == 1, contribution from u_{i, j-1}
			v = v + 1
		end
		push!(vals, v)

		# u_{i + 1, j} influences u_{ij}
		if i < n
			push!(rows, idx_i0j0)
			push!(cols, idx_ipj0)
			# u_{i+1, j} / h²
			v = one(eltype(vals))
			# -d{1, i+1/2, j} u_{i+1, j} / 2h
			v = v - d1[i, j] / 2
			push!(vals, v)
		end
		...
		# u_{i, j + 1} influences u_{ij}
		if j < m
			push!(rows, idx_i0j0)
			push!(cols, idx_i0jp)
			# u_{i, j+1} / h²
			v = one(eltype(vals))
			# -d{2, i, j+1/2} u_{i, j+1} / 2h
			v = v - d2[i, j] / 2
			push!(vals, v)
		end
...
scale_from_0_1(img) = img .+ one(real(eltype(img))) / 255

# ╔═╡ 23243cca-2a44-452a-8dad-a187663a93ba
scale_to_0_1(img) = img .- one(real(eltype(img))) / 255

# ╔═╡ 4dfc24c6-6fab-4c2d-ad26-3a9c65634521
function run_osmosis(img_input, d; abstol = 1.0e-6, reltol = 1.0e-9)
...
function drift_vector_field(img)
	...
	d2 = similar(img, (size(img, 1), size(img, 2) - 1))
...
# ╔═╡ c60d9c2a-4dba-4f43-a6ee-b1426dd60876
function merge_images(img1, idx1, img2, idx2)
	img = map(red, img1)
	d1 = drift_vector_field(scale_from_0_1(map(red, img1)))
	d2 = drift_vector_field(scale_from_0_1(map(red, img2)))
	d = (
		@. d1[1] * idx1[1:end-1, :] + d2[1] * idx2[1:end-1, :],
		@. d1[2] * idx1[:, 1:end-1] + d2[2] * idx2[:, 1:end-1]
	)
	img_red = run_osmosis(img, d)
	
	img = map(green, img1)
	d1 = drift_vector_field(scale_from_0_1(map(green, img1)))
	d2 = drift_vector_field(scale_from_0_1(map(green, img2)))
	d = (
		@. d1[1] * idx1[1:end-1, :] + d2[1] * idx2[1:end-1, :],
		@. d1[2] * idx1[:, 1:end-1] + d2[2] * idx2[:, 1:end-1]
	)
	img_green = run_osmosis(img, d)
	
	img = map(blue, img1)
	d1 = drift_vector_field(scale_from_0_1(map(blue, img1)))
	d2 = drift_vector_field(scale_from_0_1(map(blue, img2)))
	d = (
		@. d1[1] * idx1[1:end-1, :] + d2[1] * idx2[1:end-1, :],
		@. d1[2] * idx1[:, 1:end-1] + d2[2] * idx2[:, 1:end-1]
	)
	img_blue = run_osmosis(img, d)

	ColorTypes.RGB.(
		scale_to_0_1(img_red), scale_to_0_1(img_green), scale_to_0_1(img_blue)
	)
end
...
manifest_format = "2.0"
project_hash = "8b6ed239e8c1a7d617f509d76241de151cd0d9c4"

...
# ╔═╡ 9404e6af-fbe4-429a-bc00-e276b380e4b9
using QuadGK
...
md"""
# 3.2 Interpolatorische Quadratur

Wir vergleichen die zusammengesetzte Trapezregel mit der Clenshaw-Curtis-Quadratur.
"""
...
Wir verwenden die Trapezregel und die Clenshaw-Curtis-Quadratur 
...
sowie eine *exponentielle Konvergenz* der Clenshaw-Curtis-Quadratur.
"""
...
md"""
In diesem Fall zeigen beide Quadraturen eine exponentielle Konvergenz.
...
Sprungstelle hat, wird die Approximation durch die Clenshaw-Curtis-Quadratur
schlechter.
"""
...
let y = 1.0
	...
	quad_trapezoidal = map(N -> trapezoidal(x -> exp(-x^2), zero(y), y, N), Ns)
	...
	Ns = 2:20
	quad_cc = map(N -> clenshaw_curtis(x -> exp(-x^2), zero(y), y, N), Ns)
...
let
	...
	quad_trapezoidal = map(N -> trapezoidal(exp ∘ abs2 ∘ sin, 0.0, 2π, N), Ns)
	err_trapezoidal = @. abs(quad_trapezoidal - exact)
	quad_cc = map(N -> clenshaw_curtis(exp ∘ abs2 ∘ sin, 0.0, 2π, N), Ns)
	err_cc = @. abs(quad_cc - exact)
...
manifest_format = "2.0"
project_hash = "3a72346ecce4b5a41c905c825654534e040f2e18"

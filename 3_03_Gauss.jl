...
Wir vergleichen die Gauß-Legendre-Quadratur mit der 
zusammengesetzte Trapezregel und der Clenshaw-Curtis-Quadratur.
"""

# ╔═╡ f4cad97e-7c5a-4915-b569-ef5010e3151a
md"""
## Gauß'sche Glockenkurve

Wir verwenden die Quadraturen zur Approximation des Integrals

$$\int_0^1 \mathrm{e}^{-x^2} \,\mathrm{d} x$$

und berechnen den Fehler für verschiedene Anzahlen der Stützstellen $n$. An den Ergebnissen erkennt man klar die erwartete Konvergenzordung 2 der Trapezregel
sowie eine *exponentielle Konvergenz* der Gauß-Legendre- und Clenshaw-Curtis-Quadratur.
"""
...
Clenshaw-Curtis-Quadraturen schlechter.
"""
...
# ╔═╡ bf692a52-377f-4f0e-8095-486fb59746cb
let y = 1.0
	exact = 0.5 * sqrt(π) * erf(y)
	...
	# err_cc = @. max(abs(quad_cc - exact), eps())
	err_cc = @. abs(quad_cc - exact)
	scatter!(Ns, err_cc; label = "Clenshaw-Curtis")
	order = @. 2 * exp(-2 * Ns);
	lines!(Ns, order; label = L"\propto \mathrm{e}^{c n}", 
...
manifest_format = "2.0"
project_hash = "c1e5dc6f5b59ad2b48bb72d1540174c7c5ebe74f"

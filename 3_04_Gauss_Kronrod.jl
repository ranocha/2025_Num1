...
# ╔═╡ a913dd9c-9e3d-4635-932f-e364729dab3b
using Printf

# ╔═╡ a2157283-6e9f-4dc7-a670-57114a6019b1
using SpecialFunctions: erf, besselj0

# ╔═╡ fe3815ac-8432-458d-958b-62cf0337a323
using FastTransforms: clenshawcurtisnodes, clenshawcurtisweights, chebyshevmoments1

# ╔═╡ 7f58cb07-e5eb-429d-865b-1db03cab0638
using FastGaussQuadrature: gausslegendre

# ╔═╡ c293134d-376f-4753-8b5e-ac190d422a36
using QuadGK
...
# ╔═╡ 48994dcc-861b-43f8-9e0b-104f8e5b80ae
md"""
## Eine periodische Funktion

Obwohl für hinreichend glatte Funktionen die allgemeine Theorie eine Konvergenzordnung von 2 zeigt, kann die Konvergenz für gewisse Funktionen trotzdem besser sein. Man kann dies beispielsweise für periodische Funktionen (und geeignete Stützstellen) beweisen. Hier betrachten wir konkret

$$\int_0^{2\pi} \mathrm{e}^{\sin(x)^2} \,\mathrm{d}x.$$
"""

# ╔═╡ 8ccaaa83-d2ad-4640-ad33-d6f781fe724b
md"""
In diesem Fall zeigen alle Quadraturen eine exponentielle Konvergenz.
Die Trapezregel hat hier allerdings die besseren Konstanten und konvergiert
schneller.
"""

# ╔═╡ 0634a8d5-ea80-4320-9133-2e9fb6ac6325
md"""
## Eine weniger glatte Funktion

Die beiden vorherigen Beispiele behandelten analytische Funktionen, d.h.,
Funktionen, die in der ganzen komplexen Ebene in Potenzreihen entwickelt
werden können. Wenn man etwas weniger glatte Funktionen hat wie beispielsweise

$$f(x) = \mathrm{e}^{-1 / x^2},$$

dann konvergiert die Gauß-Legendre-Quadratur nicht unbedingt schneller als die
...
$$f\colon [-1, 1] \to \mathbb{R}, \qquad f(x) = |x| x^2.$$

Da die Funktion nur $C^2$ ist und die dritte Ableitung bei $x = 0$ eine
Sprungstelle hat, wird die Approximation durch die Gauß-Legendre- und 
Clenshaw-Curtis-Quadraturen schlechter. Im Gegensatz dazu ist die $h$-adaptive
Gauß-Kronrod-Quadratur gut in der Lage, das Integral zu approximieren.
"""

# ╔═╡ f8b1a3cd-d38f-4547-9a46-de3f3eef7fae
md"""
...
Da die Funktion nicht stetig differenzierbar ist, können wir nur schlechtere
Approximationen erwarten. Die adaptive Gauß-Kronrod-Quadratur schlägt sich
aber immer noch gut.
"""

# ╔═╡ 0bd116c6-658c-4ce8-9fa0-0c7a34af7ede
md"""
Das signifikant bessere Abschneiden der Gauß-Kronrod-Quadratur liegt daran, 
dass bei einer Unterteilung des Intervalls $[-\pi, \pi]$ irgendwann die 
Teilintervalle genau an den Grenzen $\pm \pi / 2$ liegen und damit die 
Quadratur nur glatte Bereiche der Funktion "sieht". Wenn wir die Grenzen
anders wählen, etwa

$$\int_{-1.23 \pi}^\pi f(x) \, \mathrm{d} x$$
...
# ╔═╡ 7202f20e-9224-491e-a99f-e22852334323
function trapezoidal(f, a, b, N)
	nodes = range(a, b; length = N)
	result = (f(a) + f(b)) / 2
	for x in nodes[(begin + 1):(end - 1)]
		result += f(x)
	end
	result *= step(nodes)
	return result
...
# ╔═╡ 68d4574e-b88f-4f15-81b5-25c95e38c4cb
function clenshaw_curtis(f, a, b, N)
	nodes_ = reverse(clenshawcurtisnodes(Float64, N))
	weights_ = clenshawcurtisweights(chebyshevmoments1(Float64, N))
	nodes = @. a + (nodes_ + 1) * (b - a) / 2
	weights = @. weights_ * (b - a) / 2
	return sum(f(nodes[i]) * weights[i] for i in eachindex(nodes))
end

# ╔═╡ 50bab88b-fa9e-4f54-832f-519f60d8e012
function gauss_legendre(f, a, b, N)
	nodes_, weights_ = gausslegendre(N)
	nodes = @. a + (nodes_ + 1) * (b - a) / 2
	weights = @. weights_ * (b - a) / 2
	return sum(f(nodes[i]) * weights[i] for i in eachindex(nodes))
end
...
let
	Ns = 3:50
	# exact = real(2 * π * besselj0(im))
	# quadrature = map(N -> trapezoidal(exp ∘ sin, 0.0, 2π, N), Ns)
	f = exp ∘ abs2 ∘ sin
	a = 0.0
	b = 2π
	exact = real(2 * π * sqrt(exp(1)) * besselj0(0.5 * im))
	quad_trapezoidal = map(N -> trapezoidal(f, a, b, N), Ns)
	err_trapezoidal = @. abs(quad_trapezoidal - exact)
	quad_cc = map(N -> clenshaw_curtis(f, a, b, N), Ns)
	err_cc = @. abs(quad_cc - exact)
	quad_gl = map(N -> gauss_legendre(f, a, b, N), Ns)
	err_gl = @. abs(quad_gl - exact)

	fig = Figure()
	ax = Axis(fig[1, 1]; xlabel = L"n", ylabel = "Fehler", yscale = log10)

	scatter!(Ns, err_trapezoidal; label = "Trapezregel")
	scatter!(Ns, err_cc; label = "Clenshaw-Curtis")
	scatter!(Ns, err_gl; label = "Gauß-Legendre")

	maxevals = 15 .+ 30 * (0:10)
	maxevals = maxevals[maxevals .<= 3 * maximum(Ns)]
	res = map(n -> quadgk_count(f, a, b; atol = eps(), maxevals = n), maxevals)
	quad_gk = first.(res)
	err_gk = @. max(abs(quad_gk - exact), eps())
	Ns_gk = last.(res)
	scatter!(Ns_gk, err_gk; label = "Gauß-Kronrod")
...
let
	...
	maxevals = 15 .+ 30 * (0:10)
	maxevals = maxevals[maxevals .<= 1.5 * maximum(Ns)]
	res = map(n -> quadgk_count(f, a, b; atol = eps(), maxevals = n), maxevals)
	quad_gk = first.(res)
	err_gk = @. max(abs(quad_gk - exact), eps())
	Ns_gk = last.(res)
...
let
	Ns = round.(Int, 10.0 .^ range(1, 6, length = 10))
	f(x) = abs(x) * x^2
	a = -1.0
	b = 1.0
	exact = 0.5
	...
	fig = Figure()
	ax = Axis(fig[1, 1]; xlabel = L"n", ylabel = "Fehler",
			  xscale = log10, yscale = log10)

	scatter!(Ns, err_trapezoidal; label = "Trapezregel")
	order = Ns.^(-2); order *= err_trapezoidal[end] / order[end]
	lines!(Ns, order; label = L"\propto n^{-2} \propto \Delta x^2", color = :gray)
	
	scatter!(Ns, err_cc; label = "Clenshaw-Curtis")
	order = Ns.^(-4); order *= err_cc[1] / order[1]
	order[order .< eps()] .= NaN
	lines!(Ns, order; label = L"\propto n^{-4} \propto \Delta x^4", color = :gray,
		   linestyle = :dot)
	
	scatter!(Ns, err_gl; label = "Gauß-Legendre")

	maxevals = 15 .+ 30 * (0:10)
	maxevals = maxevals[maxevals .<= 1 * maximum(Ns)]
	res = map(n -> quadgk_count(f, a, b; atol = eps(), maxevals = n), maxevals)
	quad_gk = first.(res)
	err_gk = @. max(abs(quad_gk - exact), eps())
	Ns_gk = last.(res)
...
let
	Ns = round.(Int, 10.0 .^ range(1, 6, length = 10))
	f(x) = ifelse(-π / 2 <= x <= π / 2, cos(x), 0.0)
	a = -π
	b = π
	exact = 2.0
	quad_trapezoidal = map(N -> trapezoidal(f, a, b, N), Ns)
...
let
	...
	maxevals = 15 .+ 30 * (0:10)
	maxevals = maxevals[maxevals .<= 2 * maximum(Ns)]
	res = map(n -> quadgk_count(f, a, b; atol = eps(), maxevals = n), maxevals)
	quad_gk = first.(res)
	err_gk = @. max(abs(quad_gk - exact), eps())
	Ns_gk = last.(res)
...
manifest_format = "2.0"
project_hash = "4113529b23db8b8ab85f502c590ea7acaa598829"

...
# ╔═╡ 4ed295f7-f225-4a82-bca1-537c470025b6
using SpecialFunctions: gamma

# ╔═╡ e6c64c80-773b-11ef-2379-bf6609137e69
md"""
# 1.2 Stabilität eines Algorithmus

Wir betrachten das Beispiel

$$f(x) = \sqrt{x + 1} - \sqrt{x}$$

und berechnen den zugehörigen relativen Fehler für zwei verschiedene Algorithmen.
Dazu verwenden wir eine Referenzlösung, die mit Fließkommazahlen einer sehr hohen Genauigkeit berechnen.
"""

# ╔═╡ 0c12d5a8-4b76-4d6e-a92b-f6862d389dc5
eps(BigFloat)

# ╔═╡ 7c5b943d-bdae-4f8f-98f0-970864f2963d
function relative_error(f, x)
	fx = f(x)
	fx_reference = f(big(x))
	return abs(fx - fx_reference) / abs(fx_reference)
end

# ╔═╡ b1b97611-7547-4b57-967d-0a5b8bf42c04
function example_sqrt(T)
	f1(x) = sqrt(x + 1) - sqrt(x)
	f2(x) = inv(sqrt(x + 1) + sqrt(x))
	
	x = exp10.(range(2 * log(eps(T)), -2 * log(eps(T)), length = 1000))
	err1 = relative_error.(f1, x)
	err2 = relative_error.(f2, x)

	fig = Figure()
	ax = Axis(fig[1, 1]; xlabel = L"x", ylabel = "Relativer Fehler",
			  xscale = log10, yscale = log10,
			  title = string(nameof(T)))
	lines!(ax, x, err1; label = L"\sqrt{x + 1} - \sqrt{x}")
	lines!(ax, x, err2; label = L"1 / (\sqrt{x + 1} + \sqrt{x})")
	lines!(ax, [x[begin], x[end]], eps(T) * [1, 1];
		   label = "eps(" * string(nameof(T)) * ")",
		   linestyle = :dot, color = :gray)
	xlims!(ax, x[begin], x[end])
...
# ╔═╡ 16e39d07-195c-4e12-8789-c03ef37779a6
example_sqrt(Float32)

# ╔═╡ 32413eb6-b4c5-493a-95a5-e69180428801
md"""
## Berechnung der Eulerschen Zahl e

Wir vergleichen die beiden Möglichkeiten

$$\mathrm{e}
    = \lim_{n \to \infty} \left( 1 + \frac{1}{n} \right)^n
    = \sum_{n = 0}^\infty \frac{1}{n!}$$

der Berechnung der Eulerschen Zahl e.
"""

# ╔═╡ 106a0304-ebc7-4d5a-8cad-3c2790297d87
function compute_e_sequence(T)
	f1(n) = (1 + one(T) / n)^n
	n = round.(Int, exp10.(range(0, 16, length = 1000)))
	e = map(f1, n)

	fig = Figure()
	fig[0, 1] = Label(fig, string(nameof(T)); tellwidth = false, font = :bold)

	n_opt = round(Int, 1 / sqrt(eps(T)))
	e_opt = f1(n_opt)
	
	ax1 = Axis(fig[1, 1]; xlabel = L"n", ylabel = L"(1 + 1 / n)^n",
			   xscale = log10)
	lines!(ax1, n, e)
	scatter!(ax1, [n_opt], [e_opt]; color = :gray, marker = :x)
	text!(ax1, L"n = 1 / \sqrt{\varepsilon}"; 
		  position=(n_opt, 1.05 * e_opt), space = :data, align = (:center, :bottom))
	
	ax2 = Axis(fig[2, 1]; xlabel = L"n", ylabel = "Absoluter Fehler",
			   xscale = log10, yscale = log10)
	lines!(ax2, n, @. abs(e - ℯ))
	scatter!(ax2, [n_opt], [abs(e_opt - ℯ)]; color = :gray, marker = :x)
	text!(ax2, L"n = 1 / \sqrt{\varepsilon}"; 
		  position=(0.4 * n_opt, 1.05 * abs(e_opt - ℯ)), space = :data, align = (:right, :center))
...
compute_e_sequence(Float32)

# ╔═╡ d1e3bebc-542b-410b-ac32-30579e2899e4
md"""
Als nächstes betrachten wir die Exponentialreihe.
"""

# ╔═╡ 20b0fddc-27ea-4d53-8014-5facc64c2513
function compute_e_series(T)
	# gamma(n) = (n - 1)!
	f1(n) = sum(one(T) / gamma(T(k + 1)) for k in 0:n)
	# f2(n) = sum(one(T) / gamma(T(k + 1)) for k in n:-1:0)
	n = 0:50
	e1 = map(f1, n)
	# e2 = map(f2, n)
	...
	ax1 = Axis(fig[1, 1]; xlabel = L"n", ylabel = L"\sum_{k = 0}^n \frac{1}{k!}")
	lines!(ax1, n, e1)
	# lines!(ax1, n, e2)

	ax2 = Axis(fig[2, 1]; xlabel = L"n", ylabel = "Absoluter Fehler",
			   yscale = log10)
	lines!(ax2, n, @. abs(e1 - ℯ))
	# lines!(ax2, n, @. abs(e2 - ℯ))
